...
class AsteroidBelt{
  ...
  rotateAsteroids(){
    for (var i = 0; i < this.asteroidsInUse.length; i++){
      let asteroid = this.asteroidsInUse[i];
      ...
      let diffPos = spaceship.mesh.position.clone().sub(asteroid.mesh.position.clone());
      ...
      if (d < game.asteroidDistanceTolerance){
        ...
        game.asteroidsArray.unshift(this.asteroidsInUse.splice(i,1)[0]);
