...
function updateAsteroids(a) {
  ...
  // laser HIT
  if (spaceship.laserOn && distanceBetweenY < 15 && spaceship.mesh.position.x < a.mesh.position.x) {
...
function handleDestroyAstro(hitAsteroid) {
  // hitAsteroid.mesh.scale.set(0.90, 0.90,0.90);

  overheadLight.intensity = 20;
  setTimeout(() => overheadLight.intensity = 0.3, 100);
  hitAsteroid.mesh.position.x = 430 + Math.random()*20;
